...
```text
Hyperliquid 的模式:
专用交易基建 + 可编程应用层 + 专业交易应用。

Lyquor 的机会:
开放构建交易基建的能力，并把它表达成被排序驱动的 Lyquid applications。
```

## Hyperliquid 的业务形态

Hyperliquid 的核心产品能力是市场结构。HyperCore 提供的是一套专用交易基建：订单簿、spot/perp 流动性、清算、资产、vault、staking，以及 oracle-driven state。这给活跃金融应用提供了很强的底层。
...
```text
HyperCore = Hyperliquid 已经做好的专用交易基建
Lyquor = 开放给开发者构建专用交易基建的能力
```

所以 Lyquor 版本不应该被描述成 “EVM contracts next to HyperCore”。它更像是把交易基础设施模块实现成被排序驱动的 Lyquid network applications，并由共享网络状态和 runtime capabilities 协调起来。
...
把账户、保证金、结算、风控和交易所服务基建构建成 Lyquid network applications，
通过 sequencing 保证订单和状态迁移顺序。
```
...
第四个优势是内部灵活性。外部用户仍然可以通过 Ethereum-compatible address、ABI-like interface、JSON-RPC、wallet signature 或 sequencing contract 交互。内部产品则可以使用 Lyquid/WASM 和 runtime capabilities，而不是受 Solidity 和 EVM execution 限制。
...
```text
Sequencing layer -> Lyquid network applications -> 开发者构建的交易基建
```

如果说 HyperEVM 让 Hyperliquid 的专用交易基建可以通过 contracts 被编程，那么 Lyquor 开放的是直接把这类交易基建构建成 sequenced network applications 的能力。也就是说，一个 HyperCall-like 产品放在 Lyquor 上，不只是拥有一个结算层。它的撮合、清算、风控、保证金、强平和 oracle-driven settlement 模块，都可以成为协同的 Lyquid applications。

这才是主要产品启发：HyperCore 是专用交易基建；Lyquor 是构建专用交易基建的开放环境。
